...
database_server = '172.28.2.208:27017'
...
def get_valid_signatures():
    logger.info('Getting valid signatures from mongo at %s', database_server)
...
def insert_bounty(squad_name, signature, bounty_value, pull_request_url, date_merged):
    ...
    logger.info('Inserting bounty into mongo at %s', database_server)
    client = MongoClient('mongodb://' + database_server)
    db = client.bounty
